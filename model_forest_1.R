...
modis<-subset(modis,year<11&year>5)
modis<-subset(modis,distance>0.5&forest>0.9)
...
formula.1 <- coordinates+year ~
  ...
  #forest(f.forest2(x,y,year),model="factor_contrast")+
  road(f.road(x,y,year),model="linear",prec.linear=0.01)+
  distance(f.distance(x,y,year),model="factor_contrast")+
...
fit1 <- lgcp(formula.1, modis,sampler=ips,domain=list(coordinates=mesh,year=6:10),options = list( safe=TRUE,inla.mode="experimental",verbose=T,
...
save.image("model_forest_2.Rdata")
