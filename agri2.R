...
raster_response <- rast("raster/response/stack_agri.tif")  # 2 layers
raster_response<-raster_response[[4:7]]
...
for (yr in seq_len(nl)) {
  ...
  df_tmp <- tibble(
    ...
    road =  factor(cov_vals$road[, yr],levels = c(1, 2, 3,4,5)),
    agri =factor(cov_vals$agri[, yr],levels = c(0,1,2,3)),
...
df_long <- df_long %>%
  mutate(
    ...
    agri = relevel(factor(agri), ref = "0"),
...
spde <- inla.spde2.pcmatern(mesh = mesh,
                            prior.range = c(50, 0.9),   # P(range < 30km) = 0.5 (tune)
...
formula_st <- y ~ mcwd+road+agri+governance+pasture_change+annual_crop_change+perrenial_crop_change+blacklist+
...
summary(res_st_agri2)
rm(list=setdiff(ls(),c("res_st_agri2","mesh","stack_st","df_long")))
save.image("model_inla_agri2.Rdata")
