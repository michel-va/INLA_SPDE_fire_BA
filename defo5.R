...
raster_response<-raster_response[[15:16]]
...
for (yr in seq_len(nl)) {
  ...
  df_tmp <- tibble(
    ...
    fragmentation=factor(cov_vals$fragmentation[,yr],levels=c(1,2,3,4,5)),
...
formula_st <- y ~ mcwd+road+agri+fragmentation+governance+pasture_change+annual_crop_change+perrenial_crop_change+blacklist+
...
summary(res_st_defo5)
rm(list=setdiff(ls(),c("res_st_defo5","mesh","stack_st","df_long")))
save.image("model_inla_defo5.Rdata")
